...
def test_enrich_calls_progress_cb():
    posts = [_make_post(i) for i in range(1, 51)]  # 50 posts
    session = MagicMock()
    session.get.return_value = _SHOWTHREAD_HTML
    scraper = _make_scraper(session)
    mock_cb = MagicMock()
    scraper.progress_cb = mock_cb
    scraper._enrich_with_full_content(posts)
    
    assert mock_cb.call_count == 50
    mock_cb.assert_called_with(50, 50, 50)


def test_enrich_calls_progress_cb_when_not_multiple_of_25():
    ...
    mock_cb.assert_called_with(10, 10, 10)
